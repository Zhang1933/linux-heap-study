...
static __thread mstate thread_arena attribute_tls_model_ie; // 线程独立arena,mstate描述一个arena。
...
/* arena_get() acquires an arena and locks the corresponding mutex.     
   ...
   in the new arena. */
// arena_get()尝试上锁并获得一个arena。最开始尝试上一次被这个线程成功锁上的arena。(通常情况这是为了速度用宏执行。) 然后，遍历一遍arena循环链表。如果没有满足要求的arena，申请一个新的。在后一种情况中，size 只是提示新arena立即需要多少的内存空间。
#define arena_get(ptr, size) do { \
      ptr = thread_arena;						      \
      arena_lock (ptr, size);						      \
  } while (0) // do while(0) 宏可以方便用于if判断,多语句，函数。 ptr指向线程的arena。到这里时thread_arena已经初始化为main_arena(308行,ptmalloc_init函数中)。
...
#define arena_lock(ptr, size) do {					      \
  ...
  } while (0) // ptr不为NUL时对arena上锁。否则，TODO
...
ptmalloc_init (void)
{
  if (__malloc_initialized >= 0) // 只会初始化一次
    return;
  ...
  malloc_init_state (&main_arena); // 创建一个arena。
...
# endif
  TUNABLE_GET (mxfast, size_t, TUNABLE_CALLBACK (set_mxfast)); // 通过设置环境变量对malloc调参:https://www.gnu.org/software/libc/manual/html_node/Tunables.html 。
#else
  const char *s = NULL;
  if (__glibc_likely (_environ != NULL)) //下面通过环境变量设置参数。
    {
      char **runp = _environ;
      char *envline;
  ...
  void (*hook) (void) = atomic_forced_read (__malloc_initialize_hook); // 弱类型 __malloc_initialize_hook 钩子函数指针初始化为NULL。
  if (hook != NULL)
    (*hook)();
#endif
  __malloc_initialized = 1;// 初始化完毕
}
