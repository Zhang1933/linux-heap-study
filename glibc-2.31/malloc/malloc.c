...
# define tidx2usize(idx)	(((size_t) idx) * MALLOC_ALIGNMENT + MINSIZE - SIZE_SZ) // 根据下标计算出对应的tcache bins中chunk的大小。tcache bins中chunk的大小随下标的关系： 64位是首项24，末项为1032,公差为16。32位是首项为12,末项为516，公差为8。 FIXME:tcache bins中最小chunk为什么和其他bins中的最小chunk大小不一样。
...
#define request2size(req)                                         \
  (((req) + SIZE_SZ + MALLOC_ALIGN_MASK < MINSIZE)  ?             \
   MINSIZE :                                                      \
   ((req) + SIZE_SZ + MALLOC_ALIGN_MASK) & ~MALLOC_ALIGN_MASK) /* 实际的大小= 对齐后的 请求大小加一个字的长度 。64位最小分配32字节。 FIXME:如果要56字节,分配64字节在使用中怎么装前两个字段? */
...
#define fastbin_index(sz) \
  ((((unsigned int) (sz)) >> (SIZE_SZ == 8 ? 4 : 3)) - 2) // fastbin 64位公差32。
...
/*
   ...
   entries if max_fast is reduced. 设置fastbin中chunk的最大值，输入0表示没有fastbin。FIXME: do_check_malloc_state是什么意思？malloc_consolidate()是什么意思？
 */
...
struct malloc_par
{
  ...
  size_t tcache_unsorted_limit; // TODO： 添加具体注释。
#endif
};
...
__libc_free (void *mem)
{
  ...
  if (mem == 0)                              /* free(0) has no effect .free(0)没有影响 */
    return;
  ...
  if (in_smallbin_range (nb)) // 如果申请的chunk的大小满足smallbins中chunk的最大大小。
    {
      ...
      if ((victim = last (bin)) != bin) // bin是链表头。如果链表不为空。
        {
          bck = victim->bk;
